...
function navLinkClass({ isActive }) {
  return `rounded-lg px-3 py-2 text-sm font-medium transition-[background-color,color] duration-150 ${isActive ? "bg-white/10 text-white" : "text-slate-400 hover:bg-white/5 hover:text-white"}`;
}
...
export default function Navbar() {
  ...
  return <header className="sticky top-0 z-50 border-b border-white/10 bg-[#0B1120]/95" style={{ backgroundColor: "rgba(11, 17, 32, 0.95)" }}>
    ...
    {isOpen && <div id="landing-navigation-drawer" className="border-t border-white/10 bg-[#0B1120]/95 px-4 py-4 lg:hidden"><div className="mx-auto flex max-w-7xl flex-col gap-1">{landingNavigation.map(({ label, to, end }) => <NavLink key={to} to={to} end={end} className={navLinkClass} onClick={closeMenu}>{label}</NavLink>)}<NavLink to="/website-review" className="mt-3 rounded-lg bg-gradient-to-r from-violet-500 to-indigo-500 px-4 py-2.5 text-center text-sm font-semibold text-white transition-colors duration-150 hover:from-violet-400 hover:to-indigo-400" onClick={closeMenu}>Start Reviewing</NavLink></div></div>}
  </header>;
}
