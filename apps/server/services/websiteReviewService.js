...
export async function createWebsiteReview(url, force = false) {
  ...
  if (!force && cached && cached.expiresAt > Date.now()) { timing("cache-hit", Date.now() - started); return { ...cached.review, cached: true }; }
  ...
  try {
    ...
    if (review) { cache.set(cacheKey, { review, expiresAt: Date.now() + CACHE_TTL }); if (cache.size > CACHE_MAX) cache.delete(cache.keys().next().value); timing("response-parse-and-validation", Date.now() - started); return { ...review, cached: false }; }
    ...
    cache.set(cacheKey, { review: corrected, expiresAt: Date.now() + CACHE_TTL }); if (cache.size > CACHE_MAX) cache.delete(cache.keys().next().value); timing("total", Date.now() - started); return { ...corrected, cached: false };
  } catch (error) {
