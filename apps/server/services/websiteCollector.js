...
async function getBrowser(onTiming) {
  if (sharedBrowser?.isConnected()) return sharedBrowser;
  if (!browserPromise) {
    const started = Date.now();
    browserPromise = chromium.launch({ headless: true }).then((browser) => {
      sharedBrowser = browser;
      browser.on("disconnected", () => { sharedBrowser = undefined; browserPromise = undefined; });
      onTiming?.("chromium-startup", Date.now() - started);
      return browser;
...
export async function closeWebsiteBrowser() { await sharedBrowser?.close().catch(() => {}); sharedBrowser = undefined; browserPromise = undefined; }
...
function collectEvidence(requestedUrl, finalUrl, html) {
  ...
  const headings = $("h1, h2, h3, h4, h5, h6").map((_, element) => ({ level: Number(element.tagName.slice(1)), text: $(element).text().replace(/\s+/g, " ").trim() })).get().filter(({ text }) => text).slice(0, 24);
  const images = $("img").map((_, element) => ({ alt: $(element).attr("alt") || "", hasAlt: $(element).attr("alt") !== undefined })).get().slice(0, 30);
  const inputs = $("input, textarea, select").map((_, element) => { const id = $(element).attr("id"); const ariaLabel = $(element).attr("aria-label"); return { type: $(element).attr("type") || element.tagName.toLowerCase(), hasLabel: Boolean(ariaLabel || (id && $(`label[for="${id}"]`).length) || $(element).closest("label").length) }; }).get().slice(0, 30);
...
async function captureScreenshot(finalUrl, onTiming) {
  let context;
  let page;
  try {
    const browser = await getBrowser(onTiming);
    ...
    await page.route("**/*", async (route) => {
      ...
      try {
        ...
        if (request.resourceType() === "media" || request.resourceType() === "font" || /google-analytics|googletagmanager|doubleclick/i.test(request.url())) return route.abort();
        return route.continue();
      } catch {
        return route.abort();
      }
    });
    const navigationStart = Date.now(); await page.goto(finalUrl, { waitUntil: "domcontentloaded", timeout: SCREENSHOT_TIMEOUT_MS }); onTiming?.("page-navigation", Date.now() - navigationStart);
    await page.waitForTimeout(700);
    const screenshotStart = Date.now(); let screenshot = await page.screenshot({ type: "jpeg", quality: 65, fullPage: false }); onTiming?.("screenshot-capture", Date.now() - screenshotStart);
    if (screenshot.toString("base64").length > MAX_SCREENSHOT_BASE64_BYTES) screenshot = await page.screenshot({ type: "jpeg", quality: 45, fullPage: false });
